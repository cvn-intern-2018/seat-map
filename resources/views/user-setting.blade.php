...
@section('content')
    ...
    <div class="container">
        <div id="id01" class="modal">
            <form name="inforPopup" class="modal-content animate" action="/users/add" method="POST"
                ...
                <div class="container_">
                    ...
                    <p><b>Email:</b></p>
                    <input name="email" value="levanduc@gmail.com" maxlength="50" type="text"
                    ...
                    @foreach($users as $user)
                        @if($user->permission == 0)
                            ...
                            <li class="flex-container_" draggable="true">
                                <p name="name" class="bg" data-info="{{ $arr_users[$user->id] }}">
                                    <strong>{{ $user->name }}</strong></p>
                                <div><img name="{{ $user->name }}" src="{{asset('images/remove.png')}}" data-id="{{$user->id}}" alt="delete"/>
                                </div>
                            </li>
                        @endif
                    @endForeach
            ...
            <div id="right" class="flex-container_">
                <form name="infor" class="flex-container_" action="/users/edit" method="POST"
                    ...
                    <div name="avatar" id="avatarcontainer_" class="flex-container_">
                        ...
                        <div class=flex-container_>
                            <button onclick="document.getElementById('changeAvar').click()" name="changeAvar"
                                    type="button">Change
                            </button>
                            <input type="hidden" value="0" name="checkAvatar"/>
                            <input type="file" accept="image/*" id="changeAvar" name="avatar"
                                   value="{{asset('images/user/'.json_decode($admin)->id.json_decode($admin)->avatar)}}"/>
                    ...
                    <div id="userinformation">
                        ...
                        <?php if (isset(session('prv_error')['fullnameErr'])) {
                            echo "<p name=\"Error\">" . (session('prv_error'))['fullnameErr'] . "</p>";
                        } ?>
                        <p><strong>Phone number:</strong></p>
                        <input type="text" name="phone" value="" maxlength="30" autocomplete="off"/>
                        <?php if (isset(session('prv_error')['phoneErr'])) {
                            echo "<p name=\"Error\">" . (session('prv_error'))['phoneErr'] . "</p>";
                        } ?>
                        <p><strong>Email:</strong></p>
                        <input type="text" name="email" value="" maxlength="50" autocomplete="off"/>
                        <?php if (isset(session('prv_error')['emailErr'])) {
                            echo "<p name=\"Error\">" . (session('prv_error'))['emailErr'] . "</p>";
                        } ?>
