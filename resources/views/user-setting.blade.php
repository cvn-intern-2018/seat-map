...
@section('content')
    ...
    <div id="id01" class="modal">
        <form name="inforPopup" class="modal-content animate" onsubmit="addUser(event)" action="/users/add"
            ...
            <ul id="list-of-users" data-old="{{session('old')}}" data-info="{{ session('user_id') }}"
                ...
                @foreach($users as $user)
                    @if($user->permission == 0)
                        <li class="flex-container">
                            <p name="name" onclick="displayUser(this)" data-info="{{ $userj[$user->id] }}">
                                <strong>{{ $user->name }}</strong></p>
                            <button name="{{ $user->name }}" onclick="deleteUser(this)" type="button"><img
                                        src="{{asset('images/remove.png')}}" alt="delete"/></button>
                        </li>
                    @endif
                @endForeach
        ...
        <div id="right" class="flex-container">
            <form name="infor" class="flex-container" action="/users/edit" method="POST"
                ...
                <div id="userinformation">
                    ...
                    <?php if (isset(session('uv')['fullnameErr'])) {
                        echo "<p name=\"Error\">" . (session('uv'))['fullnameErr'] . "</p>";
                    } ?>
                    <p><strong>Phone number:</strong></p>
                    <input type="text" name="phone" value="{{json_decode($admin)->phone}}"/>
                    <?php if (isset(session('uv')['phoneErr'])) {
                        echo "<p name=\"Error\">" . (session('uv'))['phoneErr'] . "</p>";
                    } ?>
                    <p><strong>Email:</strong></p>
                    <input type="text" name="email" value="{{json_decode($admin)->email}}"/>
                    <?php if (isset(session('uv')['emailErr'])) {
                        echo "<p name=\"Error\">" . (session('uv'))['emailErr'] . "</p>";
                    } ?>
